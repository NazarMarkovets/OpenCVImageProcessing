    ...
    public partial class MainForm : Form
    {
        ...
        private void ConfigureControls(bool visible = true, bool enable = true, string control = null)
        {
            for (int i = controls.Count - 1; i >= 0; i--)
            {
                ...
                switch (true)
                {
                    case true when ctr is CheckBox:
                    case true when ctr is Button:
                        if (ctr.InvokeRequired) ctr.Invoke(new SafeCallDelegate(ConfigureControls), new object[] { visible, enable, controls });
                        else ctr.Enabled = enable; ctr.Visible = visible;
                        break;
                    case true when ctr is CircularProgressBar.CircularProgressBar:
                        if (!progressBar.IsDisposed)
                        {
                            if (progressBar.InvokeRequired) progressBar.Invoke(new SafeCallDelegate(ConfigureControls), new object[] { visible, enable, controls });
                ...
                controls.RemoveAt(controls.IndexOf(ctr));
            }
